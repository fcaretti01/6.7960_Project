...
from utils.utils import gelu_new, solu, gelu_fast
...
# Embed & Unembed
class Embed(nn.Module): # Modify this method to replace the embedding matrix with an MLP extracting latent features from input tensor
    def __init__(self, cfg: Union[Dict, EasyTransformerConfig]):
        super().__init__()
        if isinstance(cfg, Dict):
            cfg = EasyTransformerConfig.from_dict(cfg)
        self.cfg = cfg
        self.W_E = nn.Parameter(torch.empty(self.cfg.d_in, self.cfg.d_model)) # self.W_E = nn.Parameter(torch.empty(self.cfg.d_vocab, self.cfg.d_model))

    def forward(self, tokens):
        # If A has shape [a, b] and B has shape [c, d], then A[:, B] has shape [a, c, d]
        # B acts as a tensor of indices into the second dimension (so >=0 and <b)
        emb = einsum("batch pos d_in, d_in d_model -> batch pos d_model", tokens, self.W_E) # Project input onto d_model space
        return emb #self.W_E[tokens, :]  # Shape [batch pos d_model]
